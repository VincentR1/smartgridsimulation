...
class SimpleGrid(Grid):
    def __init__(self, number_consumer=1, number_producer=1, steps=1, transportation_eff=0.8):
        ...
        producers = [SimpleProducer(steps) for i in range(number_producer)]
...
class SimpleGridWurm(Grid):
    def __init__(self, number_consumer=1, number_producer=1
                 ...
                 transport_eff=0.8):
        consumers = [SimpleConsumer(steps) for i in range(number_consumer)]
        producers = [SimpleProducer(steps) for i in range(number_producer)]
        ...
        last_grid.adding_nodes(consumers, [transport_eff] * len(consumers))
