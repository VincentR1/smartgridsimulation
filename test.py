...
def test():
    ...
    if round(updated_balance, 6) != 0.0:
        ...
        nr_test_pass += 1
    expected_loss = (1 - transport_eff ** 2) * 100
    ...
    if round(updated_balance, 6) != 0.0:
        ...
        nr_test_pass += 1
    expected_loss = (1 / transport_eff ** 2 - 1) * 100
    if round(updated_loss, 6) != expected_loss:
        print("loss should be: " + str(expected_loss) + " but is : " + str(updated_loss))
        nr_test_failed += 1
    else:
    ...
    grid3 = SimpleGridWurm(steps=1, number_producer=1, number_consumer=1, number_interconecting_grids=13,
                           transport_eff=transport_eff)
    return_flow, updated_balance, updated_loss, updated_mineff, settled = grid3.start(0)
    ...
    expected_loss = (1 - transport_eff ** 13) * 100
    if round(updated_loss, 6) != expected_loss:
        print("loss should be: " + str(expected_loss) + " but is : " + str(updated_loss))
        nr_test_failed += 1
    else:
